...
async function buildCherryinConfig(ctx: BuilderContext): Promise<ProviderConfig> {
  ...
  try {
    ...
    logger.warn('CherryIn sub-provider config unavailable', { error })
  }
...
function buildAzureConfig(ctx: BuilderContext): ProviderConfig<'azure' | 'azure-anthropic' | 'azure-responses'> {
  const modelId = ctx.model.apiModelId ?? ctx.model.id
  const endpointType = ctx.model.endpointTypes?.[0]
  ...
  const isResponsesVersion = apiVersion === 'preview' || apiVersion === 'v1'
  const isResponsesVariant = ctx.aiSdkProviderId === 'azure-responses' || isResponsesVersion
