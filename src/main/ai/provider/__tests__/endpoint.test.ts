...
describe('resolveAiSdkProviderId', () => {
  describe('Catalog adapterFamily (highest priority)', () => {
    ...
    it('returns openai-compatible when adapterFamily is unknown', () => {
      ...
      // makes no attempt to recover — writers own adapterFamily quality.
      const provider = makeProvider({
        id: 'anthropic',
        endpointConfigs: {
    ...
    it('returns openai-compatible when adapterFamily is missing entirely', () => {
      ...
      // Resolver doesn't infer from provider.id or baseUrl — endpoint config
      // writers are responsible for setting adapterFamily at write time.
      const provider = makeProvider({
        id: 'anthropic',
        endpointConfigs: {
  ...
  describe('Catalog-backed registered extensions', () => {
    // Seeded catalog providers arrive with `endpointConfigs[ep].adapterFamily`
    // set. Fixtures mirror that shape.
  ...
  describe('Relay-style multi-endpoint provider shape', () => {
    // MiniMax is a registered catalog provider whose endpoints carry
    // adapterFamily independently.
    ...
    it('routes anthropic-messages endpoint to the anthropic adapter (REGRESSION)', () => {
      ...
      // anthropic-protocol endpoints. Fix: every endpoint can carry its own
      // adapterFamily from the provider catalog.
